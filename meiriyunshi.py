...
def get_today_fortune():
    ...
    request_data = {
        "model": "deepseek-chat",
        "messages": [{"role": "user", "content": "生成一段励志图文，要求：你就是毛泽东本人亲自对二三十岁的在低谷期和身处绝境的年青人加油打气，点燃他们心中的希望，可以包含毛泽东语录诗词选集内容等，也可加毛泽东亲身经历等故事，用亲切幽默的口吻，内容一定要做到给人希望启迪智慧的效果，每天生成的内容必须不同。"}],
        "temperature": 0.8,
        "max_tokens": 500
    }
    try:
...
def push_to_wechat(content):
    ...
    push_data = {
        ...
        "title": f"今日运势 🚀 {datetime.now().strftime('%Y-%m-%d')}",
        "content": content,
        "template": "txt"
    }
    try:
